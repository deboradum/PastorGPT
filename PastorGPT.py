...
class Pastor:
    def __init__(self, local, save_convo, pdf_file="bible.pdf", txt_file="bible.txt", db_name="bible.pkl"):
    ...
    def save_conversation(self, query, answer, sources):
        if self.save_path is None:
            i = len([f for f in os.listdir("conversations/") if f.startswith("bible")])
            self.save_path = f"conversations/bible-conversation{i:02d}.json"
    ...
    def create_faiss_db(self, chunks):
        ...
        faiss.write_index(db.index, "chunks.index")
        db.index = None
